/**@@@+++@@@@******************************************************************
 **
 ** Windows Interceptor frida script v1.0 hyugogirubato
 **
 ** frida "C:\Program Files\StreamFab\StreamFab\StreamFab64.exe" -l windows.js
 **
...
// Harcoded modules
const MODULES = [{"name": "sub_14000BC30", "address": "0x14000BC30", "size": 108}, {"name": "sub_14000BCA0", "address": "0x14000BCA0", "size": 1189}, {"name": "sub_14000DF50", "address": "0x14000DF50", "size": 615}]
 
// Custom params
const PACKAGE = "C:\\Program Files\\StreamFab\\StreamFab"; // undefined for intercept everything
const LIBRARIES = ["StreamFab64.exe"]; // empty for intercept everything
const INCLUDES = []; // empty for intercept everything, "^" and/or "$" filter short functions according to regex
...
const searchLibraries = () => {
    ...
    if (LIBRARIES.length > 0) {
        return libraries.filter(lib => LIBRARIES.some(library => lib["path"].toLowerCase().includes(library.toLowerCase())));
    } else {
        return libraries;
    }
}
...
const searchModules = (library) => {
    ...
    modules = modules.length > 0 ? modules : MODULES.map(module => ({ ...module, address: ptr(module["address"]) })); // PATCH: modules
...
setTimeout(function () {
    console.log("---");
    console.log("Capturing Windows app...");
